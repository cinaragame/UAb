...
void PrintListaInts(Lista *lista, int n, char *str, FILE *f)
{
	int i;
	if(f == NULL)
		return;
	...
	for(i = 0; i < n; i++)
	{
		fprintf(f, ";%d", lista->num);
		lista = lista->next;
	}
	fprintf(f, ";\n");
}

void PrintWhileSorting(Lista *lista, FILE *f)
{
	if(f == NULL)
		return;
	fprintf(f, "Lista:");
...
Lista *InsertSort(Lista *lista, FILE *f)
{
	int counter = 0;
	...
	while(pt != NULL)
	{
		...
		if(counter <= 10)
			PrintWhileSorting(ordered_list, f);
		pt = pt->next;
	}
	while(lista != NULL)
		lista = Remove(lista);
...
int main()
{
	...
	f = fopen("insertsort.csv", "a");
    scanf("%d",&n);
    for(i = 0; i < n; i++)
        lista = Adiciona(lista, randaux() % 1000);
    PrintListaInts(lista, 10, "Lista nao ordenada:", f);
    lista = InsertSort(lista, f);
    PrintListaInts(lista, 10, "Lista ordenada:", f);
    while(lista != NULL)
        lista = Remove(lista);
	fclose(f);
}
